...
class EventController{
    static async purchaseTickets(req, res) {
        ...
        try {
          ...
          if (!ticket) {
            return res.status(400).json({ message: "Ticket non disponible ou période de vente expirée" });
          }
      
          // Vérifier la disponibilité
          if (ticket.available < quantity) {
            return res.status(400).json({ message: "Quantité de tickets insuffisante" });
          }
          ...
          try {
            ...
            return res.status(200).json({ message: "Achat réussi", tickets: pdfName });
          } catch (err) {
